...
func (client *Client) Status(ctx context.Context, stationID string) (gateway.CoffeeStationStatus, error) {
	request, err := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		client.baseURL+"/stations/"+url.PathEscape(stationID)+"/status",
		nil,
	)
	if err != nil {
