...
Tracer-bullet implementation of a mandatory MCP Enforcement Boundary. Authenticated clients can call narrow Tools for the demo coffee station, a minimized calendar Free/Busy View and an exact Owner-approved Meeting Proposal flow. The Go gateway validates signed identity, composes a Security Context, obtains an OPA Policy Decision, calls network-isolated simulated Protected Resources, validates their responses, and returns structured MCP content with the OPA `decision_id` and policy revision.
...
- Both Actors may read `demo-station` and receive `state: ready` through policy revision `ticket-04`.
